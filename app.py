...
def create_gradio_interface(config_path="config/config.yaml"):
    ...
    with gr.Blocks(css_paths='styles.css', theme='ParityError/Interstellar') as demo:
        ...
        gr.HTML("4. Record or Upload Audio (Max: 30 seconds)", elem_classes=["instruction"], padding=False)
        ...
        gr.HTML("Note: This demo runs on an on-demand GPU hosting service, which requires some time to load the model before processing. As a result, operations may take up to 5 minutes to complete. Please be patient while the model initializes and generates results.", elem_classes=["note"], padding=False)
